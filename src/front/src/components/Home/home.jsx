...
const Home = () => {
  ...
  useEffect(() => {
    if(isLoggedIn === true){
        fetch(`http://3.144.215.154:8080/apointment/getNewApointments?clientId=${user.email}`).then(response => {
          response.json().then(data => {
            setnewApointments(data);
          });
        })
        fetch(`http://3.144.215.154:8080/apointment/getPastApointments?clientId=${user.email}`).then(response => {
          response.json().then(data => {
